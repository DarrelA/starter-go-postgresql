...
func TestLoggerMW(t *testing.T) {
	...
	t.Run("does not log request or response bodies", func(t *testing.T) {
		app := fiber.New()
		var buf bytes.Buffer
		log.Logger = log.Output(&buf)
		t.Cleanup(func() { log.Logger = log.Output(os.Stdout) })
		zerolog.SetGlobalLevel(zerolog.InfoLevel)

		app.Use(func(c *fiber.Ctx) error {
			c.Locals("requestID", "request-id")
			c.Locals("correlationID", "correlation-id")
			c.Locals("env", "dev")
			return c.Next()
		})
		app.Post("/login", LoggerMW, func(c *fiber.Ctx) error {
			return c.JSON(fiber.Map{"access_token": "response-secret-token"})
		})

		req := httptest.NewRequest("POST", "/login?code=query-secret-code", strings.NewReader(`{"password":"request-secret-password"}`))
		req.Header.Set("Content-Type", "application/json")
		req.Header.Set("Referer", "https://client.example/callback?token=referer-secret-token")
		resp, err := app.Test(req)
		if err != nil {
		...
		output := buf.String()
		for _, secret := range []string{"request-secret-password", "response-secret-token", "query-secret-code", "referer-secret-token"} {
			if strings.Contains(output, secret) {
				t.Fatalf("log output contains sensitive value %q: %s", secret, output)
			}
		}
		for _, field := range []string{"request_body_bytes", "response_body_bytes"} {
			if !strings.Contains(output, field) {
				t.Fatalf("log output does not contain %q: %s", field, output)
			}
		}
	})
